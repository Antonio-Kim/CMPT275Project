...
open class MetronomeChartView: MacawView {
    ...
    private let gradientColor = LinearGradient(degree: 90, from: Color(val: 0xc2e59c), to: Color(val: 0x64b3f4))
    ...
    private func createAnimations() {
        ...
        for (index, node) in mainGroup.contents.enumerated() {
            if let group = node as? Group {
                let heightValue = self.barHeight / 100 * (5*barsValues[index])
